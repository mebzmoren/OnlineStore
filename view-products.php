  ...
  if(isset($_SESSION['member_id'])) {
    ...
    if ($check['quantity'] >= $quantity_bought) {
      ...
      if ($payment_optional != NULL) {
        if (empty($address) || empty($city) || empty($phone_number) || empty($payment_optional) || empty($quantity_bought)) {
      ...
      if (isset($payment_type)) {
        if (empty($address) || empty($city) || empty($phone_number) || empty($payment_type) || empty($quantity_bought)) {
          $error = 'Please fill the required fields.';
          header('location: ' . $_SERVER['REQUEST_URI']);
          die();
        } else {
    ...
    $error = 'You need to login as a member in order to buy.';
  }
}
...
if (isset($_POST['rate-product'])) {
  if(isset($_SESSION['member_id'])) {
    ...
    $error = 'You need to login as a member in order to rate.';
  }
}
